...
class FourSectionCutGenerator:
    ...
    def calculate_four_section_parameters(self, hole_diameter, advance_length):
        ...
        df = 3.2 * math.sqrt(advance_length**2)
        ...
        St4 = math.sqrt(2) / 2 * (math.sqrt(2) / 2 * B1 * math.sqrt(2) / 2 + B2 + B3)
        sections.append({
            'burden': B4,
            'spacing': X4,
            'stemming': St4,
    ...
    def generate_stoping_pattern(self, center, tunnel_radius, cut_holes, hole_diameter):
        ...
        num_rings = int((tunnel_radius - cut_extent) / 0.5)  # 0.5m spacing between rings
    ...
    def generate_contour_holes(self, center, tunnel_radius, hole_diameter):
        ...
        positions = self.acad.calculate_points_on_circle(center, contour_radius, num_holes)
    ...
    def draw_drilling_pattern(self, holes, tunnel_outline=None):
        ...
        for hole in holes:
            ...
            # Add label
            if 'label' in hole:
                text_pos = (hole['position'][0] + hole['diameter'], 
                          hole['position'][1] + hole['diameter'])
                self.acad.draw_text(text_pos, hole['label'], 0.08, 
    ...
    def add_pattern_info(self, center, tunnel_radius, advance_length, hole_diameter):
        ...
        self.acad.draw_text(title_pos, "FOUR-SECTION CUT DRILLING PATTERN", 0.2, 7)
...
def main():
    ...
    try:
        ...
        contour_holes = generator.generate_contour_holes(center, radius, hole_diameter)
